...
int** fillArray(unsigned int numRows, unsigned int numCols)
{
	...
	My2DArray = new int*[numRows]; // assigns the address of a new array to My2DArray
	...
	for (int i = 0; i < numRows; i++) {
		...
		//and 2) assign it to the current row of your array.
										//<=== PUT YOUR CODE HERE!
		My2DArray[i] = new int[numCols]; // creates new arrays and sets their address to the address of each index of My2dArray
...
void deleteArray(int** A, int size)
{
	...
	{
		delete A[i]; // deletes the arrays within My2DArray's indexes
	}
	delete A; // deletes My2DArray
